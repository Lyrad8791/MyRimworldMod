...
namespace RimWorld
{
    public class WorkGiver_DoBill_Advanced: WorkGiver_Scanner
    {
        ...
        private static WorkGiver_DoBill_Advanced.DefCountList availableCounts = new WorkGiver_DoBill_Advanced.DefCountList();
        ...
        {
            WorkGiver_DoBill_Advanced.MissingSkillTranslated = "MissingSkill".Translate();
            WorkGiver_DoBill_Advanced.MissingMaterialsTranslated = "MissingMaterials".Translate();
        }
        ...
        private Job StartOrResumeBillJob(Pawn pawn, IBillGiver giver)
        {
            for (int i = 0; i < giver.BillStack.Count; i++)
            {
                ...
                {
                    if (Find.TickManager.TicksGame >= bill.lastIngredientSearchFailTicks + WorkGiver_DoBill_Advanced.ReCheckFailedBillTicksRange.RandomInRange || FloatMenuMakerMap.makingFor == pawn)
                    {
                        ...
                        if (bill.ShouldDoNow())
                        {
                            if (bill.PawnAllowedToStartAnew(pawn))
                            {
                                ...
                                {
                                    JobFailReason.Is(WorkGiver_DoBill_Advanced.MissingSkillTranslated);
                                }
                                else
                                {
                                    ...
                                    if (bill_ProductionWithUft != null)
                                    {
                                        if (bill_ProductionWithUft.BoundUft != null)
                                        {
                                            ...
                                            return WorkGiver_DoBill_Advanced.FinishUftJob(pawn, bill_ProductionWithUft.BoundUft, bill_ProductionWithUft);
                                        }
                                        else
                                        {
                                            UnfinishedThing unfinishedThing = WorkGiver_DoBill_Advanced.ClosestUnfinishedThingForBill(pawn, bill_ProductionWithUft);
                                            if (unfinishedThing != null)
                                            {
                                                return WorkGiver_DoBill_Advanced.FinishUftJob(pawn, unfinishedThing, bill_ProductionWithUft);
                                            }
                                        }
                                    }
                                    if (WorkGiver_DoBill_Advanced.TryFindBestBillIngredients(bill, pawn, (Thing)giver, this.chosenIngThings))
                                    {
                                    ...
                                    {
                                        JobFailReason.Is(WorkGiver_DoBill_Advanced.MissingMaterialsTranslated);
                                    }
                                }
                            }
                        }
        ...
        private bool isGenderApropriate(Pawn pawn,Bill bill)
        {
            
            if (bill.recipe.defName[0] == 'S' && bill.recipe.defName[0] == 'x' && pawn.gender == Gender.Male)
            {
                return false;
            }
            else
        ...
        {
            chosen.Clear();
            WorkGiver_DoBill_Advanced.newRelevantThings.Clear();
            if (bill.recipe.ingredients.Count == 0)
            {
                return true;
            }
            IntVec3 rootCell = WorkGiver_DoBill_Advanced.GetBillGiverRootCell(billGiver, pawn);
            Region rootReg = rootCell.GetRegion(pawn.Map, RegionType.Set_Passable);
            if (rootReg == null)
            {
                return false;
            }
            WorkGiver_DoBill_Advanced.MakeIngredientsListInProcessingOrder(WorkGiver_DoBill_Advanced.ingredientsOrdered, bill);
            WorkGiver_DoBill_Advanced.relevantThings.Clear();
            WorkGiver_DoBill_Advanced.processedThings.Clear();
            bool foundAll = false;
            ...
            if (billGiverIsPawn)
            {
                WorkGiver_DoBill_Advanced.AddEveryMedicineToRelevantThings(pawn, billGiver, WorkGiver_DoBill_Advanced.relevantThings, baseValidator, pawn.Map);
                if (WorkGiver_DoBill_Advanced.TryFindBestBillIngredientsInSet(WorkGiver_DoBill_Advanced.relevantThings, bill, chosen))
                {
                    return true;
                }
            }
            ...
            WorkGiver_DoBill_Advanced.processedThings.AddRange(WorkGiver_DoBill_Advanced.relevantThings);
            RegionProcessor regionProcessor = delegate (Region r)
            {
                ...
                {
                    Thing thing = list[i];
                    if (!WorkGiver_DoBill_Advanced.processedThings.Contains(thing))
                    {
                        if (ReachabilityWithinRegion.ThingFromRegionListerReachable(thing, r, PathEndMode.ClosestTouch, pawn))
                        {
                            ...
                            {
                                WorkGiver_DoBill_Advanced.newRelevantThings.Add(thing);
                                WorkGiver_DoBill_Advanced.processedThings.Add(thing);
                            }
                        }
                    }
                }
                regionsProcessed++;
                if (WorkGiver_DoBill_Advanced.newRelevantThings.Count > 0 && regionsProcessed > adjacentRegionsAvailable)
                {
                    ...
                    WorkGiver_DoBill_Advanced.newRelevantThings.Sort(comparison);
                    WorkGiver_DoBill_Advanced.relevantThings.AddRange(WorkGiver_DoBill_Advanced.newRelevantThings);
                    WorkGiver_DoBill_Advanced.newRelevantThings.Clear();
                    if (WorkGiver_DoBill_Advanced.TryFindBestBillIngredientsInSet(WorkGiver_DoBill_Advanced.relevantThings, bill, chosen))
                    {
                        foundAll = true;
        ...
        {
            MedicalCareCategory medicalCareCategory = WorkGiver_DoBill_Advanced.GetMedicalCareCategory(billGiver);
            List<Thing> list = map.listerThings.ThingsInGroup(ThingRequestGroup.Medicine);
            WorkGiver_DoBill_Advanced.tmpMedicine.Clear();
            for (int i = 0; i < list.Count; i++)
            {
                Thing thing = list[i];
                ...
                {
                    WorkGiver_DoBill_Advanced.tmpMedicine.Add(thing);
                }
            }
            WorkGiver_DoBill_Advanced.tmpMedicine.SortBy((Thing x) => -x.GetStatValue(StatDefOf.MedicalPotency, true), (Thing x) => x.Position.DistanceToSquared(billGiver.Position));
            relevantThings.AddRange(WorkGiver_DoBill_Advanced.tmpMedicine);
            WorkGiver_DoBill_Advanced.tmpMedicine.Clear();
        }
        ...
        private static bool TryFindBestBillIngredientsInSet(List<Thing> availableThings, Bill bill, List<ThingAmount> chosen)
        {
            ...
            {
                return WorkGiver_DoBill_Advanced.TryFindBestBillIngredientsInSet_AllowMix(availableThings, bill, chosen);
            }
            return WorkGiver_DoBill_Advanced.TryFindBestBillIngredientsInSet_NoMix(availableThings, bill, chosen);
        }
        ...
        private static bool TryFindBestBillIngredientsInSet_NoMix(List<Thing> availableThings, Bill bill, List<ThingAmount> chosen)
        {
            ...
            chosen.Clear();
            WorkGiver_DoBill_Advanced.availableCounts.Clear();
            WorkGiver_DoBill_Advanced.availableCounts.GenerateFrom(availableThings);
            for (int i = 0; i < WorkGiver_DoBill_Advanced.ingredientsOrdered.Count; i++)
            {
                IngredientCount ingredientCount = recipe.ingredients[i];
                bool flag = false;
                for (int j = 0; j < WorkGiver_DoBill_Advanced.availableCounts.Count; j++)
                {
                    float num = (float)ingredientCount.CountRequiredOfFor(WorkGiver_DoBill_Advanced.availableCounts.GetDef(j), bill.recipe);
                    if (num <= WorkGiver_DoBill_Advanced.availableCounts.GetCount(j))
                    {
                        if (ingredientCount.filter.Allows(WorkGiver_DoBill_Advanced.availableCounts.GetDef(j)))
                        {
                            if (ingredientCount.IsFixedIngredient || bill.ingredientFilter.Allows(WorkGiver_DoBill_Advanced.availableCounts.GetDef(j)))
                            {
                                for (int k = 0; k < availableThings.Count; k++)
                                {
                                    if (availableThings[k].def == WorkGiver_DoBill_Advanced.availableCounts.GetDef(j))
                                    {
                                        ...
                                        if (num2 > 0)
                                        {
                                            ...
                                            {
                                                flag = true;
                                                float num4 = WorkGiver_DoBill_Advanced.availableCounts.GetCount(j);
                                                num4 -= (float)ingredientCount.CountRequiredOfFor(WorkGiver_DoBill_Advanced.availableCounts.GetDef(j), bill.recipe);
                                                WorkGiver_DoBill_Advanced.availableCounts.SetCount(j, num4);
                                                break;
                                            }
                                        }
                                    }
